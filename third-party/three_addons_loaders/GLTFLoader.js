	...
	loadTexture( textureIndex ) {
		...
		if ( ! loader ) {
			...
			if ( json.extensionsRequired && json.extensionsRequired.indexOf( this.name ) >= 0 ) {
				...
				throw new ( 'GLTFLoader: setKTX2Loader must be called before loading KTX2 textures' );
...
function addPrimitiveAttributes( geometry, primitiveDef, parser ) {
	...
	if ( THREE.ColorManagement.workingColorSpace !== THREE.LinearSRGBColorSpace && 'COLOR_0' in attributes ) {
		...
		console.warn( `GLTFLoader: Converting vertex colors from "srgb-linear" to "${THREE.ColorManagement.workingColorSpace}" not supported.` );
