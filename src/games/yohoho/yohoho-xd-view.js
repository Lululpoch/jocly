	...
	View.Game.xdInit = function (xdv) {
		...
		xdv.createGadget("panel",
		{
			"cartoon3d" :
			{
				...
				create : function()
				{
					const panel_geometry = new THREE.PlaneGeometry(10, 10)
					const panel_material = new THREE.MeshBasicMaterial(
						{
							color : 0xffffff,
							side : THREE.DoubleSide,
							transparent : true
						})
					const panel = new THREE.Mesh(panel_geometry, panel_material)

					return panel
				},
				click : function()
				{
					xdv.updateGadget("panel", {
						"cartoon3d":
						{
							z : 6000,
							opacity : 1
		...
		*/
		...
		class MenuButton {
				...
				options = {})
			{
				...
				this.options[this.parentMenu.skin] = {}
				this.options[this.parentMenu.skin].visible = true
				this.options[this.parentMenu.skin].type = "custommesh3d"
				this.options[this.parentMenu.skin].create = function ()
				{
					return b_this.model
				}
				this.options[this.parentMenu.skin].click = this.onClick

				// this.gadget = this.xdv.createGadget(this.buttonName, this.options)

				this.parentMenu.xdv.createGadget(this.buttonName, this.options)
				this.gadget = this.parentMenu.xdv.gadgets[this.buttonName]
			}
		}
		...
		class MenuObject {
				...
				options = {})
			{
				...
				subMenus.forEach(submenu => {
					if (submenu && submenu instanceof Gadget) {
				...
				buttons.forEach(button => {
					if (button && button instanceof Gadget) {
				...
				this.zClosed = zClosed
				this.zOpened = zOpened
				this.animationDuration = animationDuration
				this.closeButton = this.addButton(menuName + "CloseButton", [], function () {m_this.closeMenu()})
				...
				this.options[this.skin] = {}
				this.options[this.skin].visible = true
				this.options[this.skin].type = "custommesh3d"
				this.options[this.skin].create = function ()
				{
					return m_this.model
				}

				//just for tests
				this.options[this.skin].click = function ()
				{
					console.log("clicked on menu")
				}


				// this.gadget = this.xdv.createGadget(this.menuName, this.options)
				this.xdv.createGadget(this.menuName, this.options)
				this.gadget = this.xdv.gadgets[this.menuName]
				console.log("this.gadget : ", this.gadget)
			}
			...
			addMenu(
				...
				options = {})
			{
				let subMenu = new MenuObject (this.xdv, this, subMenuName, this.skin, subMenus, buttons, model, zClosed, zOpened, animationDuration, options)
				if (subMenu)
				{
					this.subMenus.push(subMenu)
				}
			...
			addButton(
				...
				options = {})
			{
				let button = new MenuButton (this, buttonName, location, onClick, model, options)
				if (button) {
					this.buttons.push(button)
				}
				return button
			...
			closeMenu ()
			{
				if (this.opened)
				{
					this.xdv.updateGadget(this.menuName,
						{
							"cartoon3d":
							{
								z: this.zClosed,
		...
		// tests
		let m = new MenuObject(xdv, null, "systemMenu", "cartoon3d")
		m.openMenu()
		/* console.log("m : ", m)
		console.log("m.gadget : ", m.gadget)
		console.log("m.closeButton : ", m.closeButton)
		console.log("m.closeButton.gadget : ", m.closeButton.gadget) */
